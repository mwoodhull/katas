...
def test_numerals_to_int_hundreds_returns_correct_response():

    assert RomanNumerals.from_roman("CXIII") == 113

    assert RomanNumerals.from_roman("CCCXLIX") == 349

    assert RomanNumerals.from_roman("CDXCIV") == 494
...
def test_numerals_to_int_thousands_returns_correct_response():
    ...
    assert RomanNumerals.from_roman("MCDLIX") == 1459
...
def test_numerals_to_int_101_and_1001_returns_correct_response():
    ...
    assert RomanNumerals.from_roman("MI") == 1001

    assert RomanNumerals.from_roman("CI") == 101
